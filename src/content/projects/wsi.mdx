...
def function_to_approximate(x: npt.NDArray[np.float64]) -> npt.NDArray[np.float64]:
    return (np.sin(x * PARAMETERS[0])
            + np.cos(x * PARAMETERS[1])
            )


EVAL_X: npt.NDArray[np.float64] = np.linspace(
    L_BOUND,
    U_BOUND,
    100
)
...
def unison_shuffled_copies(a: npt.NDArray[np.float64], b: npt.NDArray[np.float64]) \
    ...
    assert len(a) == len(b)
    p: npt.NDArray[np.intc] = np.random.permutation(len(a))
    return a[p], b[p]
...
class Network:
    def __init__(self, hidden_layer_sizes: List[int] = []):
        ...
        self.weights.append(
            np.zeros(shape=(self.sizes[-1], self.sizes[-2]))
        )
        ...
        self.biases.append(np.zeros(shape=(self.sizes[-1], 1)))
    ...
    def predict(self, x: float) -> float:
        activations: npt.NDArray[np.float64] = np.array([[x]])
    ...
    def mean_loss(self) -> float:
        ...
        return loss(
            np.array([self.predict(x) for x in EVAL_X]),
            EVAL_Y).mean()
    ...
    def mini_batch(self,
        ...
        gradient_weights: List[npt.NDArray[np.float64]] = [
            np.zeros(shape=weight.shape)
            for weight in self.weights
        ]
        gradient_biases: List[npt.NDArray[np.float64]] = [
            np.zeros(shape=bias.shape)
    ...
    def backward(self, x: float, expected_y: float) \
        ...
        y: npt.NDArray[np.float64] = np.array([[expected_y]])

        activation: npt.NDArray[np.float64] = np.array([[x]])
